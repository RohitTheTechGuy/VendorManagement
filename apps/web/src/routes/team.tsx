...
export function TeamPage() {
  ...
  const inputCls = "mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-indigo-500";
                ...
                {members.map((m) => (
                  <tr key={m.id} className="border-b border-slate-100 last:border-0">
                    <td className="px-4 py-3 font-medium text-slate-800">
                      {m.fullName ?? "—"}
                      {m.isSelf && <span className="ml-2 text-xs text-slate-400">(you)</span>}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        className={cn(
                          ...
                          m.isSelf ? "cursor-not-allowed text-slate-300" : "text-rose-600 hover:bg-rose-50",
                        )}
        ...
        <Card className="h-fit p-6">
          ...
          <form onSubmit={onAdd} className="mt-4 space-y-4">
            ...
            <label className="block text-sm font-medium">
              ...
              {fieldErrors.fullName && <span className="mt-1 block text-xs text-rose-600">{fieldErrors.fullName[0]}</span>}
            </label>
            <label className="block text-sm font-medium">
              Email
            ...
            <label className="block text-sm font-medium">
              ...
              <span className="mt-1 block text-xs text-slate-400">Share this with the teammate so they can sign in.</span>
            </label>
